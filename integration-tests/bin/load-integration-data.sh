...
MAPPING_LOAD_CMD="curl -fsS -H \"Content-Type: application/x-ndjson\" -XPUT ${ELASTIC_HOST}/glossaryv1 --data-binary \"@glossary-es-mapping.json\""
mapping_output=$(eval $MAPPING_LOAD_CMD)
echo $mapping_output

echo "Load the records"
BULK_LOAD_CMD="curl -fsS -H \"Content-Type: application/x-ndjson\" -XPOST ${ELASTIC_HOST}/_bulk --data-binary \"@glossary-data.jsonl\""
